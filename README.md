...
2D 反応機構 (`.rxn`) と sidecar TOML config から、反応点 (anchor) を中心とした全球面 (4π sr) Fibonacci サンプリング + steric blocking filter (角度シャドウ + d_min ceiling) で初期方向を生成し、**Stage 1 人工力 screening → top-K → Stage 2 CI-NEB** の二段パイプラインで 3D 反応経路を評価し、Blender で ball-and-stick アニメーションを生成するパイプライン。

目的は妥当なアニメーション (正確な TS エネルギーは目標としない)。**Phase 8 で CI-NEB は canonical 評価**となり、人工力 relax は Stage 1 screening (endpoint 生成器) として常時実行される。対応反応:
...
> **Phase 8 で削除されたフラグ**: `--neb-refine`、`--neb-images`、`--model`。モデル指定 / NEB パラメータ / 並列度はすべて TOML の `[model]` `[neb]` `[parallel]` セクションに移行。
...
- `out/<rxn>/trajectory.xyz` — best NEB trial (lowest peak_energy) trajectory
- `out/<rxn>/energies.json` — best NEB trial の image energies
- `out/<rxn>/meta.json` — placement 統計 + screening_trials 全件 + top_k_indices + neb_results 全件 + wall_clock_breakdown
...
Phase 8 から `[model]` / `[neb]` / `[parallel]` セクションが追加された (省略時は default 値):
...
| `.rxn` | description | formed (map) | broken (map) | k_form | k_broken | r_broken (Å) | max_relax_steps | r_form | n_candidates | CI-NEB |
|---|---|---|---|---|---|---|---|---|---|---|
| sn2.rxn | SN2 anion (`O⁻ + CH₃Cl`) | `[[1,3]]` | `[[1,2]]` | 0.5 | 1.0 | 4.0 | 100 | 元素表 | 64 | 全反応で適用 (top_k=4) |
| proton_transfer.rxn | Proton transfer (`HCl + NH₃`) | `[[1,3]]` | `[[1,2]]` | 1.0 | 2.0 | 4.0 | 100 | 1.05 | 64 | 全反応で適用 (top_k=4) |
| menshutkin.rxn | Menshutkin (`NH₃ + CH₃Cl`) | `[[1,5]]` | `[[5,9]]` | 4.0 | 2.0 | 5.0 | 200 | 元素表 | 64 | 全反応で適用 (top_k=4) |
| e2.rxn | E2 elimination | `[[4,5]]` | `[[2,5],[1,3]]` | 1.0 | 1.0 | 4.0 | 200 | 元素表 | 64 | 全反応で適用 (top_k=4) |
| sn1_dissoc.rxn | SN1 step 1 解離 | `[]` | `[[1,5]]` | 0.0 | 2.0 | 6.0 | 200 | — | **1** | top_k=1 (unimolecular) |
| sn1_recomb.rxn | SN1 step 2 recombination | `[[1,5]]` | `[]` | 1.0 | 0.0 | 4.0 | 200 | 元素表 (C-Cl 1.78) | 64 | 全反応で適用 (top_k=4) |

`r_form` は省略時に Cordero (2008) 共有結合半径表で per-bond ルックアップ、scalar で全 formed 同値、list で per-bond 指定。`[sampling]` は省略可能で `n_candidates=64` がデフォルト。各反応とも反応点 (anchor) を中心とした全球面 (4π sr) Fibonacci サンプリングで初期方向を生成し、ステリック blocking (角度シャドウ + d_min ceiling) を通過した方向すべてを Stage 1 で人工力 relax → 上位 K 件を Stage 2 CI-NEB で本評価する。`n_candidates` を `[sampling]` で調整可能 (例: ステリックに混雑した anchor で生存数が少ない場合は大きく)。
...
1. `reactx run examples/sn2.rxn -o out/sn2/ --backend uma --render`
   → `meta.json` で `selected_trial >= 0`, `neb_results` ≥1 件、`out/sn2/scene.blend` を Blender GUI で開いて Walden 反転を視認
...
5. `reactx run examples/sn1_dissoc.rxn -o out/sn1d/ --backend uma`
   → unimolecular auto-clamp で `screening_trials` が 1 件、`top_k=1` で NEB も 1 job
6. `reactx run examples/sn1_recomb.rxn -o out/sn1r/ --backend uma --render`
   → bimolecular で 64 候補、blocking 後の生存全件を Stage 1 で screen、top-4 を CI-NEB で本評価
...
- E2 / SN1 dissoc / SN1 recomb も CI-NEB で本評価される (Phase 7 までの 1 formed + 1 broken 限定 guard は撤廃)
...
詳細仕様: `docs/superpowers/specs/2026-05-05-cineb-unified-design.md`
実装計画: `docs/superpowers/plans/2026-05-05-cineb-unified.md`

前提となる generic placement 設計: `docs/superpowers/specs/2026-05-04-generic-placement-design.md`

## Wall-clock (実測)

> Phase 8 wall-clock 実測は Task 13 で更新予定。Phase 7 値 (RTX 5070 Ti + UMA-m-1p1、`n_candidates=64`) は `git log` 参照。

参考までに Phase 7 (人工力 relax のみ、NEB なし) の実測値:
...
UMA model load (~25–30 s) が固定コスト。Phase 8 では Stage 1 (人工力) + Stage 2 (CI-NEB top-K) で wall-clock が増える分、TS 領域が CI-NEB で精緻化される。

各実行の placement 結果 (`n_candidates` / `n_blocked` / `n_valid`)、screening trial 全件スコア、top_k_indices、neb_results、wall_clock_breakdown は `out/<rxn>/meta.json` に残る。
...
詳細設計: `docs/superpowers/specs/2026-05-05-cineb-unified-design.md`
実装計画: `docs/superpowers/plans/2026-05-05-cineb-unified.md`
